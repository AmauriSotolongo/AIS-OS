# Index — Amauri Brain Wiki

Master catalog of all wiki pages. Updated on every ingest.
Format: `[[Page]] — one-line summary (N sources)`

---

## Entities — People

- [[Amauri Sotolongo]] — Co-Founder & CTO de 1Klick, co-fundador de Digital Compass, speaker de IA en LATAM (1 source)
- [[brian-chesky|Brian Chesky]] — Co-founder y CEO de Airbnb; referencia en founder mode, diseño de producto y liderazgo detallado (1 source)
- [[hiroki-asai|Hiroki Asai]] — Creative Director de Apple bajo Steve Jobs; fuente de los principios de simplicidad y craft (1 source)
- [[santiago-aparicio|Santiago Aparicio]] — Co-founder de OnTop, ex-Rappi; referencia principal en ventas B2B para startups LATAM (1 source)

## Entities — Companies & Products

- [[1Klick]] — SaaS de automatización de ventas para PyMEs LATAM: Meta Ads + WhatsApp CRM (1 source)
- [[Digital Compass]] — Consultora de IA para PyMEs en México y LATAM (1 source)
- [[OnTop]] — SaaS de payroll internacional; abrió 110 países en 6 meses; caso de estudio de escalamiento agresivo (1 source)

## Concepts

- [[Knowledge Compounding]] — Principio de que un knowledge base crece en valor de forma exponencial por sus conexiones (1 source)

### Liderazgo & Founder Mindset
- [[founder-mode|Founder Mode]] — Estar en los detalles, no sobre-delegar; el antídoto al CEO profesional que pierde el timón (1 source)
- [[ai-founder-mode|AI Founder Mode]] — Versión AI del founder mode: menos meetings, más async, menos capas, todos hybrid manager-IC (1 source)
- [[manage-through-work|Manage Through Work]] — Lideras a través del trabajo, no como terapeuta; los pure people managers no sobreviven AI (1 source)
- [[pipeline-recruiting|Pipeline Recruiting]] — Construir pipeline de talento continuo en vez de búsquedas reactivas (1 source)
- [[motivacion-intrinseca-vs-adulacion|Motivación intrínseca vs Adulación]] — Hacer por amor al trabajo vs buscar validación externa; la trampa del éxito (1 source)
- [[progressive-overload-como-disciplina|Progressive Overload como Disciplina]] — 1% mejor cada día, consistencia sobre intensidad; lecciones del bodybuilding (1 source)
- [[miedo-como-el-fuego|El miedo como el fuego]] — El miedo no se elimina, se dirige: modo consumo vs. modo combustible (1 source)
- [[one-piece-cumples-tu-sueno-ayudando|Cumples tu sueño ayudando a otros a cumplir el suyo]] — La tripulación de Luffy: cada quien se une a su propio sueño (1 source)
- [[dropi-analogia-rio-ninos|La analogía del río y los niños]] — Los problemas son niños ahogándose; solo los ves si estás adentro del río (1 source)
- [[negocios-relaciones-no-tecnologia|El negocio es de relaciones, no de tecnología]] — La comunidad hace perdurar lo que la tecnología no puede (1 source)
- [[callar-aprender-ganar|Callar, aprender, ganar]] — El ciclo de Denzel Washington para founders: silencio primero, después explicar (1 source)
- [[liderazgo-aprender-antes-de-liderar|Aprender antes de liderar]] — El liderazgo empieza con humildad, no con órdenes (1 source)
- [[founder-empresas-son-jardines|Las empresas son jardines, no edificios]] — Mueren por falta de foco, no de dinero (1 source)
- [[producto-3-filtros|El producto correcto pasa 3 filtros]] — Moat, founder fit y narrativa de vida inevitable (1 source)
- [[talento-despedir-temprano|Nunca es demasiado pronto para despedir]] — Esperar solo hace más daño (1 source)
- [[mentalidad-dificil-no-imposible|Lo difícil no es imposible]] — Si lo crees imposible nunca lo intentas (1 source)

### Producto & Experiencia
- [[11-star-experience|11-Star Experience]] — Framework: imaginar 10 estrellas para que 6-7 sea alcanzable; ahí vive el PMF (1 source)
- [[1-to-10-to-many|1 to 10 to Many]] — Pilotear en 1 mercado → 10 → industrializar; hacer problema pequeño primero (1 source)
- [[simplicity-como-distilacion|Simplicity como Destilación]] — Simplicidad = entender la esencia, no quitar cosas; legado de Hiroki Asai/Apple (1 source)

### Marketing & Messaging
- [[storybrand-sb7|StoryBrand SB7]] — El cliente es el héroe, tu marca es el guía; Grunt Test de 5 segundos para landings (1 source)

### Estrategia & Mercado
- [[consumer-ai-renaissance|Consumer AI Renaissance]] — La ola consumer de AI llega en 12-24 meses; hoy todo es enterprise (1 source)
- [[community-over-app|Community Over App]] — Apps → agentes; la comunidad es el único moat que perdura en era AI (1 source)

### Ventas — Sistema y Proceso
- [[prospectar-vs-convencer|Prospectar vs Convencer]] — Los mejores vendedores no convencen, prospectan; venderle a quien ya está comprando (1 source)
- [[volumen-como-ventaja|Volumen como Ventaja]] — 10-30x más acciones que el promedio, sin cambiar la tasa de conversión (1 source)
- [[estandar-de-actuacion-ventas|Estándar de Actuación en Ventas]] — Definir el máximo posible en un día y convertirlo en el mínimo esperado (1 source)
- [[input-vs-output-metrics-ventas|Input vs Output Metrics en Ventas]] — Medir acciones diarias, no solo resultados; reportes por hora (1 source)
- [[founder-led-sales|Founder Led Sales]] — La math del 5x; el founder debe vender primero y documentar el proceso (1 source)
- [[tolerancia-al-dolor-ventas|Tolerancia al Dolor en Ventas]] — El rechazo no es obstáculo, es el proceso; el playbook elimina la incertidumbre (1 source)
- [[framework-24-horas|Framework de las 24 Horas]] — 24 horas de trabajo real enfocado (con cronómetro) resuelve cualquier problema (1 source)
- [[four-minute-miles|Four Minute Miles]] — Eventos internos que demuestran que lo "imposible" es posible; resetean el mindset colectivo (1 source)
- [[estacionalidad-es-bullshit|La Estacionalidad es Bullshit]] — Cuando cae la conversión, la respuesta es más funnel, no esperar (1 source)
- [[ai-en-ventas-potencializa|AI en Ventas Potencializa]] — AI amplifica volumen; el que usa AI te deja sin trabajo (1 source)

### Ventas — Estrategia & Mercado
- [[ventas-agencias-no-operaciones|Vende a agencias, no a operaciones]] — Marketing tiene budget y autonomía (1 source)
- [[mercado-agencias-mediocridad|El mercado de agencias está saturado de mediocridad]] — Las buenas se diferencian por sus herramientas (1 source)
- [[distribucion-modelo-monday|El modelo Monday]] — El cliente que paga Y distribuye (1 source)
- [[mercado-el-bote-correcto|El bote importa más que el remo]] — El mercado correcto pesa más que el esfuerzo (1 source)
- [[innovacion-no-reinventar-rueda|No reinventes la rueda]] — La innovación real rara vez parte de cero (1 source)

### Ideas — 1Klick & Visión
- [[idea-ciclo-vida-lead-1klick|Ciclo de vida completo del lead en 1Klick]] — De contenido a cierre, todo en una plataforma (1 source)
- [[idea-producto-global-desde-latam|Construir un producto global desde LATAM]] — El reto que más orgullo dará (1 source)

## Topics

- [[brian-chesky-redesigning-airbnb-ai-era|Brian Chesky — Redesigning Airbnb for the AI Era]] — Podcast completo: founder mode, 11-star experience, Project Hawaii, consumer AI renaissance (1 source)
- [[motor-ventas-predecible-aparicio|Motor de Ventas Predecible — Santiago Aparicio]] — Podcast: volumen, estándar de actuación, input metrics, four minute miles, AI en ventas (1 source)
- [[llm-wiki-pattern|LLM Wiki Pattern]] — Patrón para construir second brains con LLMs: wiki persistente vs RAG clásico (1 source)
- [[art-of-war-insights|The Art of War — Insights]] — 3 principios de Sun Tzu aplicados a la estrategia y batalla de los 50 clientes de 1Klick (1 source)
- [[landing-page-patterns-yc|Landing Page Patterns — YC Companies]] — 10 patrones de hero, social proof, CTA y pricing que comparten Stripe, Airbnb, Deel y 6 más (1 source)

## Syntheses

---

*Last updated: 2026-05-14*
